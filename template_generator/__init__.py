def generate_layout_template(dsl_app):
    ...
    header_tag = f'''<header class="fluid-container">
                    ...
                    <div class="container-fluid">
                        ...
                        <div class="collapse navbar-collapse" id="navbarNavAltMarkup">
                            <div class="navbar-nav">
                                <a class="nav-link active" href="{{{{ url_for('bp_basic.home') }}}}">{app_title}</a>\n\t\t\t\t\t\t\t'''
    for route in dsl_app['routes']:
        template_name = route['endpoint'][1:].replace('/', '_')
        route_title = route.get('title') or ''
        header_tag += f'''<a class="nav-link" href="{{{{ url_for('bp_basic.{template_name}') }}}}">{route_title}</a>\n\t\t\t\t\t\t\t'''
    header_tag += '''\n\t\t\t\t\t\t</div>
                        </div>
                    </div>
                </nav>
    ...
    scripts = '''<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha3/dist/js/bootstrap.bundle.min.js"></script>
        <script src="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.1.1/js/all.min.js"></script>
        <script src="https://code.jquery.com/jquery-3.7.0.min.js"></script>
        {% block scripts %}
        {% endblock %}'''
    layout_content = f'''<!DOCTYPE html>
<html lang="pt-br" data-bs-theme="{app_theme}">
    {head_tag}
...
def generate_home_template(dsl_app):
    app_title = dsl_app.get('title') or 'Default'
    home_content = f'''{{% extends "layout.html" %}}
...
exception_template_content = '''{% extends "layout.html" %}
